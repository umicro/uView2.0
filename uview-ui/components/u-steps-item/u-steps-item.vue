<template>
	...
	>
		...
		<view
		    ...
		    :class="[`u-steps-item__wrapper--${parentData.direction}`]"
		>
			<slot name="icon">
				<view
	...
	export default {
		...
		computed: {
			lineStyle() {
				...
				style.backgroundColor = this.parent.children[this.index + 1].error ? uni.$u.color.error : this.index < this.parentData
