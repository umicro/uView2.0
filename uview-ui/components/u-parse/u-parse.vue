...
<script>
	...
	export default {
		...
		methods: {
			...
			navigateTo(id, offset) {
				return new Promise((resolve, reject) => {
					...
					if (!id) {
						...
						this.$refs.web.evalJs('uni.postMessage({data:{action:"getOffset",offset:(document.getElementById(' + id +
							')||{}).offsetTop}})')
					}
					// #endif
					// #ifndef APP-PLUS-NVUE
				...
				(function traversal(nodes) {
					for (var i = 0; i < nodes.length; i++) {
						...
						if (node.type == 'text')
							...
							// 块级标签前后加换行
							var isBlock = node.name == 'p' || node.name == 'div' || node.name == 'tr' || node.name == 'li' || (node.name[0] ==
			 ...
			 */
			_set(nodes, append) {
				this.$refs.web.evalJs('setContent(' + JSON.stringify(nodes) + ',' + JSON.stringify([this.bgColor, this.errorImg,
					this.loadingImg, this.pauseVideo, this.scrollTable, this.selectable
				]) + ',' + append + ')')
			},
